  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 bg-background/95 backdrop-blur border-b border-border">
        <div className="max-w-5xl mx-auto flex items-center justify-between px-4 h-16">
          <div className="flex items-center gap-3">
            <img src={helvelittLogo} alt="HelveLitt" className="h-12 w-12 object-contain" />
            <span className="text-xl font-black tracking-tight text-foreground">HelveLitt</span>
          </div>
          <div className="flex items-center gap-3">
            <LanguageToggle />
            <Link to="/auth">
              <Button variant="ghost" size="sm" className="font-semibold">{t("auth.signIn")}</Button>
            </Link>
            <Link to="/auth">
              <Button size="sm" className="font-bold">{t("auth.signUp")}</Button>
            </Link>
          </div>
        </div>
      </header>
      ...
      <section className="max-w-4xl mx-auto px-4 pt-20 pb-16 text-center">
        <img src={helvelittLogo} alt="HelveLitt" className="h-24 w-24 mx-auto mb-6 object-contain" />
        <h1 className="text-5xl sm:text-6xl font-black tracking-tight text-foreground mb-4">
          HelveLitt
        </h1>
        <p className="text-xl sm:text-2xl font-medium text-primary italic mb-3">
          {t("landing.slogan")}
        </p>
      ...
      <section className="max-w-5xl mx-auto px-4 pb-20">
        <div className="grid md:grid-cols-3 gap-6">
          {features.map((f) => (
            <div key={f.titleKey} className="rounded-xl bg-card border border-border p-6 text-center">
      ...
      {/* Pricing */}
      <section className="max-w-5xl mx-auto px-4 pb-20">
        <h2 className="text-3xl font-black text-center text-foreground mb-10">{t("landing.pricing")}</h2>
        <div className="grid md:grid-cols-3 gap-6">
          {plans.map((plan) => (
            <div
              key={plan.nameKey}
            ...
            >
              ...
              <p className="text-3xl font-black text-primary mb-6">{t(plan.priceKey)}</p>
